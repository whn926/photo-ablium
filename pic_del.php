...
header("content-type:text/html;charset=UTF-8");
	...
	$pic_path=$_GET['pic_path'];//获得图片路径
	$ablum=$_GET['ablum']; //获得相册名
		require "conf.php";
	$sql1="delete from imginfo where pic_path='$pic_path' "; 
	$res1=mysql_query($sql1);
	
	if($res1==false)
		echo "<script>alert('删除照片失败！');window.location.href='photo_show.php?ablum=".$ablum."';</script>";
	else {
		
    	unlink($pic_path);
		echo "<script>alert('删除照片成功');window.location.href='photo_show.php?ablum=".$ablum."';</script>";
	}
?> 
	    ...
	    if($files){
	        foreach($files as $k=>$v){
	           ...
	           <button  class="buttonsetup" onclick="Preview('<?php echo $filepath ?>')">
	      		<?php 
	      		echo "<img width=180 height=180 src=".$filepath.">" ?>	      			
	      		</button>
	      		<a id='ablumdet'  href="<?php echo "pic_del.php?ablum=".$filepath ?> " >X</a>
